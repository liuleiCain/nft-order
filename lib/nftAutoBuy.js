(function (module, exports) {
    ...
    class NftAutoBuy {
        constructor(networkName, provider, taskStorage) {
            this.emitter = new EventEmitter()
            this.taskList = {}
            this._networkName = networkName
            this._web3Provider = provider
            this._web3 = new Web3(this._web3Provider)
            this._seaport = new OpenSeaPort(provider, {
        ...
        async _getGemOrdersList(taskName) {
            ...
            try {
                ...
                let {data} = await axios({
                    ...
                    data: {
                        "filters": {
                            ...
                            "sellOrders": 1,
                            "startingPrice": 1
                        }, "offset": 0, "limit": 5, "markets": [], "status": ["buy_now"]
                    }
                })
                ...
                // let {address} = this._web3.eth.accounts.privateKeyToAccount(task.privateKey)
                for (let v of data.data) {
                    let hash = await this._checkAndBuyNft(task.account, task.contract, v.id)
                    if (!hash) continue;
        ...
        */
        async _checkAndBuyNft(account, contract, tokenId) {
            try {
                ...
                //get sell order
                let order = await this._seaport.api.getOrder({
                    side: OrderSide.Sell,
                ...
                return await this._seaport.fulfillOrder({
                    order,
